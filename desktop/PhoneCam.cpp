...
static void stop() {
    ...
    SetWindowTextW(connectButton,L"Подключить");SetWindowTextW(statusLabel,L"Камера отключена");
...
static void start(bool test) {
    ...
    if(!test){
        if(address.empty()||address.find_first_of(L"\"\r\n\t ")!=std::wstring::npos){MessageBoxW(window,L"Введи IP телефона или полный адрес из приложения.",L"PhoneCam",MB_ICONINFORMATION);return;}
        if(address.find(L"?code=")==std::wstring::npos){
            if(code.size()!=6||code.find_first_not_of(L"0123456789")!=std::wstring::npos){MessageBoxW(window,L"Введи шестизначный код с экрана телефона.",L"PhoneCam",MB_ICONINFORMATION);return;}
    ...
    if(!registration()){MessageBoxW(window,L"Не удалось зарегистрировать камеру для текущего пользователя.",L"PhoneCam",MB_ICONERROR);return;}
    ...
    if(!CreateProcessW(exe.c_str(),command.data(),NULL,NULL,FALSE,CREATE_NO_WINDOW|CREATE_SUSPENDED,NULL,directory.c_str(),&startup,&child)) {CloseHandle(childJob);MessageBoxW(window,L"Не удалось запустить приёмник.",L"PhoneCam",MB_ICONERROR);return;}
    ...
    SetWindowTextW(connectButton,L"Отключить");EnableWindow(addressEdit,FALSE);EnableWindow(codeEdit,FALSE);
    SetWindowTextW(statusLabel,test?L"Тестовый сигнал":L"Подключаюсь к телефону…");
}
static void discover() {
    EnableWindow(findButton,FALSE);SetWindowTextW(statusLabel,L"Ищу телефон в локальной сети…");
    HWND target=window;
    std::thread([target]{
...
static void tray(bool add) {
    ...
    icon.hIcon=LoadIconW(instance,MAKEINTRESOURCEW(201));wcscpy_s(icon.szTip,L"PhoneCam — камера телефона");
...
static LRESULT CALLBACK previewProc(HWND h,UINT msg,WPARAM w,LPARAM l) {
    if(msg==WM_PAINT){PAINTSTRUCT ps;HDC dc=BeginPaint(h,&ps);RECT r;GetClientRect(h,&r);FillRect(dc,&r,(HBRUSH)GetStockObject(BLACK_BRUSH));
        ...
        }else{SetBkMode(dc,TRANSPARENT);SetTextColor(dc,RGB(170,185,195));SelectObject(dc,font);DrawTextW(dc,L"Здесь появится видео",-1,&r,DT_CENTER|DT_VCENTER|DT_SINGLELINE);}
...
static LRESULT CALLBACK procedure(HWND h,UINT msg,WPARAM w,LPARAM l) {
    ...
    case WM_CREATE:{
        ...
        control(L"STATIC",L"Включи камеру на телефоне · подключись к тому же Wi-Fi",0,24,64,590,26);
        control(L"STATIC",L"Адрес телефона",0,24,104,390,22);
        addressEdit=control(L"EDIT",L"",ES_AUTOHSCROLL|WS_TABSTOP,24,130,420,30,10);
        findButton=control(L"BUTTON",L"Найти телефон",WS_TABSTOP,454,129,154,32,11);
        wchar_t saved[512]={};GetPrivateProfileStringW(L"PhoneCam",L"Address",L"",saved,512,configFile.c_str());SetWindowTextW(addressEdit,saved);SendMessageW(addressEdit,EM_SETCUEBANNER,TRUE,(LPARAM)L"Например: 192.168.1.42:8080");
        control(L"STATIC",L"Код с телефона",0,24,176,200,22);
        codeEdit=control(L"EDIT",L"",ES_AUTOHSCROLL|ES_NUMBER|WS_TABSTOP,24,202,150,32,12);SendMessageW(codeEdit,EM_SETLIMITTEXT,6,0);
        connectButton=control(L"BUTTON",L"Подключить",WS_TABSTOP|BS_DEFPUSHBUTTON,188,201,180,34,13);
        control(L"BUTTON",L"Тест камеры",WS_TABSTOP,380,201,130,34,14);
        control(L"BUTTON",L"В трей",WS_TABSTOP,520,201,88,34,15);
        statusLabel=control(L"STATIC",L"Готово. Камера PhoneCam зарегистрирована для твоего пользователя.",0,24,250,584,44);
        preview=control(L"PhoneCamPreview",L"",0,24,304,584,328);
        control(L"STATIC",L"В приложении для звонков выбери «PhoneCam». Закрытие окна останавливает передачу.",0,24,646,584,46);
    ...
    case DiscoveryMessage:{auto result=(std::wstring*)l;EnableWindow(findButton,TRUE);if(!result->empty()){SetWindowTextW(addressEdit,result->c_str());SetWindowTextW(statusLabel,L"Телефон найден. Введи код с его экрана.");SetFocus(codeEdit);}else SetWindowTextW(statusLabel,L"Не найден. Включи передачу на телефоне или введи IP вручную.");delete result;return 0;}
    case WM_TIMER:
    ...
    case TrayMessage:
        ...
        if(l==WM_RBUTTONUP){HMENU menu=CreatePopupMenu();AppendMenuW(menu,MF_STRING,20,L"Открыть PhoneCam");AppendMenuW(menu,MF_STRING,21,L"Выключить и выйти");POINT p;GetCursorPos(&p);SetForegroundWindow(h);TrackPopupMenu(menu,TPM_RIGHTBUTTON,p.x,p.y,0,h,NULL);DestroyMenu(menu);}return 0;
...
int WINAPI wWinMain(HINSTANCE h,HINSTANCE,LPWSTR command,int show) {
    ...
    if(!prepare()){if(wcscmp(command,L"--install")==0||wcscmp(command,L"--uninstall")==0)return 3;MessageBoxW(NULL,L"Не удалось распаковать PhoneCam. Закрой приложения, использующие старую версию камеры, и повтори запуск.",L"PhoneCam",MB_ICONERROR);return 1;}
    ...
    if(!registration()){MessageBoxW(NULL,L"Не удалось зарегистрировать виртуальную камеру.",L"PhoneCam",MB_ICONERROR);return 2;}
